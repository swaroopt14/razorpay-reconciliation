...
import { DEMO_SMOKE_BATCH_ID, isDemoQuery } from '@/services/payout-command/demo/ycDemoConstants'
...
function IntentsRedirect() {
  ...
  useEffect(() => {
    ...
    q.set('dock', 'grid')
    const batch = params.get('batch_id') || params.get('client_batch_id') || DEMO_SMOKE_BATCH_ID
    q.set('batch_id', batch)
    q.set('client_batch_id', batch)
    const filter = params.get('filter')
    if (filter) q.set('filter', filter)
    if (isDemoQuery(params.get('demo'))) q.set('demo', 'sandbox') // DEMO_QUERY_VALUE
