...
/**
 * Live workspace credentials for the signed-in user (zord-edge `/v1/auth/me`).
 * Returns tenant id + key prefix only. Full API secrets are never returned or stored.
 */
    ...
    const res = NextResponse.json(
      {
        message:
          edgeResponse?.status === 401
            ? 'Sign in to load live workspace credentials.'
