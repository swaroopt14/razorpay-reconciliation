...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[zord] support tickets GET failed', err)
    // Soft-fail so the Support tab never surfaces a JSON parse / empty-body banner.
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[zord] support tickets POST failed', err)
    return NextResponse.json(
      { code: 'SUPPORT_STORE_ERROR', message: 'Could not create support ticket.' },
