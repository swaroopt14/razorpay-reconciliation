...
/**
 * Platform admin entry — same light AuthSplitLayout as /signin and /register.
 * Keeps workspace + ADMIN role gate; redirects to /admin/tenants.
 */
export default function AdminLoginPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      if (envelope.user.role !== 'ADMIN') {
        await logout()
        setError('This entry is for platform administrators only. Use an admin account.')
        setLoading(false)
        return
      }
  ...
  return (
    <AuthSplitLayout
      ...
      subtitle="Govern tenants and access with role-locked credentials. Production MFA stays required for sensitive actions."
      footer={
    ...
    >
      ...
      <Link href="/ops/login" className={`${authOutlineButtonClass} mt-2`}>
        Ops sign in
      </Link>
    </AuthSplitLayout>
  )
