/**
 * Proof Center exports — prefer evidence BFF download; fall back to local pack payload.
 */
...
function downloadProofPackJsonLocal(pack: ProofPack) {
  const payload = packToExportPayload(pack)
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' })
  triggerDownload(blob, `${pack.id.toLowerCase()}-evidence-pack.json`)
}

function downloadProofPackPdfLocal(pack: ProofPack) {
  const lines = [
    'Zord Evidence Pack Statement',
    ...
    '',
    'Integrity verification does not attest upstream bank/ERP truthfulness alone.',
  ]
...
export function downloadProofPackJson(pack: ProofPack) {
  void downloadEvidencePackJson(pack.id).then((result) => {
    if (!result.ok) downloadProofPackJsonLocal(pack)
  })
}

export function downloadProofPackPdf(pack: ProofPack) {
  void downloadEvidencePackPdf(pack.id).then((result) => {
    if (!result.ok) downloadProofPackPdfLocal(pack)
  })
}
