...
export function OutcomeReviewSurface() {
  ...
  const queue = useMemo(() => {
    ...
    return items.filter((i) => {
      ...
      // Return/reversal category includes both Returned and Reversed
      if (gap === 'return_reversal') {
  ...
  if (!ready) {
    return (
      <div className="bg-[#F8FAFC] px-5 py-5 sm:px-6">
        ...
        <div className="mt-5">
          <AwaitingUploadsEmptyState title="No outcome exceptions yet" readiness={readiness} />
        </div>
      </div>
    )
  }
    ...
    <div className="bg-[#F8FAFC] pb-10">
      ...
      <div className="border-b border-[#E5E5E5] bg-[#F8FAFC] px-5 py-4 sm:px-6">
        <LifecycleSummaryStrip
          ...
          heroHint="Potential exposure until classified · not labelled as loss"
          cells={[
            {
            ...
            },
            {
              ...
              hint: 'Provider return with reason',
            },
            {
              label: 'Unresolved',
