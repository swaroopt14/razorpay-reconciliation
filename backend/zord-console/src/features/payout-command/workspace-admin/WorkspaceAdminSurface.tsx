...
export function WorkspaceAdminSurface() {
  ...
  const [signIns, setSignIns] = useState<SignInAuditRow[]>([])
  const [signInSource, setSignInSource] = useState('')
  const [signInBackend, setSignInBackend] = useState('')
  const [signInFetchedAt, setSignInFetchedAt] = useState('')
  ...
  const loadSignIns = useCallback(async () => {
    ...
    setSignInError(null)
    try {
      const res = await fetch('/api/prod/login-audit?limit=100', { cache: 'no-store' })
      const data = (await res.json()) as SignInAuditPayload
      if (!res.ok) {
        setSignIns([])
        setSignInError(data.message || data.error || `Could not load sign-in log (${res.status}).`)
        return
      }
      setSignIns(Array.isArray(data.items) ? data.items : [])
      setSignInSource(data.source || '')
      setSignInBackend(data.status?.backend || data.source || '')
      setSignInFetchedAt(data.fetched_at || '')
    } catch {
      setSignIns([])
      setSignInError('Could not reach the login-audit API.')
    } finally {
  ...
  function exportAudit() {
    const blob = new Blob(
      [JSON.stringify({ source: signInSource, backend: signInBackend, items: signIns }, null, 2)],
      { type: 'application/json' },
    )
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = 'zord-signin-audit.json'
    a.click()
    URL.revokeObjectURL(url)
    flash('Exported live sign-in log from the login-audit API')
  }
  ...
  function raiseTicket() {
    ...
    void createSupportTicketRemote({
      ...
      topic: row.subject,
      description: `${row.severity} · ${ctx.kind} ${ctx.ref}. Support cannot expose unmasked financial data by default.`,
      priority: ticketSeverity === 'Urgent' || ticketSeverity === 'High' ? 'urgent' : 'normal',
    })
      .then(({ slackDelivered }) => {
        flash(
          slackDelivered
            ? 'Support request raised and posted to Slack'
            : 'Support request saved. Slack did not accept the webhook — check the incoming-webhook URL.',
        )
      })
      .catch((e) => {
        flash(e instanceof Error ? e.message : 'Ticket saved locally, Slack notify failed.')
      })
  }
        ...
        <div className="mt-5">
          ...
          {tab === 'audit' ? (
            <SignInAuditTable
              ...
              source={signInSource}
              fetchedAt={signInFetchedAt}
              onRefresh={() => void loadSignIns()}
              exportRows={exportAudit}
            />
          ) : null}
