...
export function DispatchRelaySurface() {
  ...
  const attachedPolicy = useBatchPolicy(dispatchedBatchId ?? undefined)
  ...
  const listStats = useMemo(() => {
    ...
    const dispatchedValue = sealed.reduce((s, r) => s + r.amountRupees, 0)
    ...
    return (
      <div className="bg-[#F8FAFC] pb-10">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-6">
          <AwaitingUploadsEmptyState title="No dispatch attempts yet" readiness={readiness} />
        </div>
      </div>
    )
  }
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          {view === 'list' ? (
            <p className="text-[13px] text-[#64748B]">
              <span className="font-semibold text-[#0B1324]">{rows.length}</span> payouts
            </p>
          ) : null}
        </div>
        {notice ? (
        ...
        {view === 'list' ? (
          <div className="mx-auto max-w-[1120px] space-y-4 p-4 sm:p-6">
            <LifecycleSummaryStrip
              ...
              heroHint={`${listStats.payoutCount} sealed payouts · sandbox rail · live data not affected`}
              cells={[
                {
                  label: 'Payouts',
                  value: String(listStats.payoutCount),
            ...
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border border-[#E5E5E5] bg-white px-4 py-3">
              <span className="flex items-center gap-2 text-[13px] text-[#0B1324]">
                <span className="font-mono font-semibold">{dispatchedBatchId}</span>
                ...
                {[
                  {
                    source: 'Zord agent',
                    ai: true,
                    text: `Batch ${dispatchedBatchId} dispatched from Intent Journal - request hashes match sealed contracts.`,
                    at: 'just now',
                  },
                  {
                    source: 'System',
