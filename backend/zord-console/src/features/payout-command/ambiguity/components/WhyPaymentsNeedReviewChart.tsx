...
import { toDisplayPercent } from '../../shared/formatApiKpiFields'
...
export function WhyPaymentsNeedReviewChart({ amb }: WhyPaymentsNeedReviewChartProps) {
  const data = useMemo(() => {
    const confPct = toDisplayPercent(amb?.avg_attachment_confidence) ?? 0
    const lowConf =
      toDisplayPercent(amb?.low_confidence_rate) ??
      (amb ? Math.max(0, 100 - confPct) : 0)
    const collision = toDisplayPercent(amb?.candidate_collision_rate)
    return [
      { label: 'Review Rate', pct: toDisplayPercent(amb?.ambiguity_rate) ?? 0, color: '#000000', pending: false },
      { label: 'Low Confidence', pct: lowConf, color: '#94a3b8', pending: false },
      {
        label: 'Missing Refs',
        pct: toDisplayPercent(amb?.provider_ref_missing_rate) ?? 0,
        color: '#334155',
        pending: false,
      },
      {
  ...
  const ambiguityRatePct = toDisplayPercent(amb?.ambiguity_rate)
    ...
    <article className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-6 flex items-baseline gap-3">
        <p className="text-[2rem] font-bold tabular-nums text-slate-900">
          {ambiguityRatePct != null ? `${ambiguityRatePct.toFixed(1)}%` : '-'}
        </p>
