...
export function PaymentGapsSurface() {
  ...
  const filteredRows = useMemo(
    () => filterGapRows(DEMO_GAP_ROWS, filters),
    [filters, DEMO_GAP_ROWS],
  )
  const categories = useMemo(
    () => categoriesFromRows(filteredRows),
    [filteredRows, DEMO_GAP_CATEGORIES],
  )
  const heroValue = useMemo(
    () => valueRequiringReview(filteredRows),
    [filteredRows, DEMO_GAP_CATEGORIES],
  )
  ...
  function openCategory(id: GapCategoryId) {
    router.push(outcomeReviewHref(id, filters))
  }
    ...
    return (
      <div className="min-h-0 flex-1 overflow-y-auto bg-[#F4F6F9]">
        <div className="mx-auto w-full max-w-[1120px] space-y-5 px-5 py-6 sm:px-6 lg:px-8">
          ...
          <AwaitingUploadsEmptyState title="No potential exposure data yet" readiness={readiness} />
        </div>
      </div>
    )
  }
      ...
      <div className="mx-auto w-full max-w-[1120px] px-5 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mt-4">
          <LifecycleSummaryStrip
            ...
            heroHint={`Potential exposure across ${filteredRows.length} payout${filteredRows.length === 1 ? '' : 's'} in the current filter · not classified as loss`}
            cells={[...categories]
              .sort((a, b) => b.valueRupees - a.valueRupees)
              .slice(0, 4)
              .map((cat) => ({
                label: cat.label,
                value: formatGapsInr(cat.valueRupees),
                hint: `${cat.payoutCount} payout${cat.payoutCount === 1 ? '' : 's'} · open Outcome Review`,
              }))}
          />
        </div>
        ...
        >
          <div className="border-b border-[#E5E7EB] px-5 py-3.5">
            ...
            <p className="mt-0.5 text-[12px] text-[#6B6B6B]">
              Click a category to open filtered Outcome Review rows
            </p>
          </div>
        ...
        <div className="mt-4 overflow-hidden rounded-lg border border-[#E5E7EB] bg-white">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#E5E7EB] px-5 py-3.5">
            <div>
              ...
              <p className="mt-0.5 text-[12px] text-[#6B6B6B]">
                Every amount links to underlying Outcome Review rows
              </p>
            </div>
            <Link
              href={outcomeReviewHref('', filters)}
                ...
                <tbody>
                  {filteredRows.map((r) => (
                    <tr key={r.id} className="border-b border-[#F0F0F0] last:border-0">
                      ...
                      <td className="px-4 py-3">
                        <Link
                          href={outcomeReviewHref(r.categoryId, filters, r.paymentRef)}
