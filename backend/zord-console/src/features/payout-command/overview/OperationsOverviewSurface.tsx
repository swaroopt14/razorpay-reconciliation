...
import { useDemoBatchReady, useBatchDispatched } from '@/services/payout-command/demo/demoBatchReadiness'
...
export function OperationsOverviewSurface() {
  ...
  const dispatched = useBatchDispatched()
  const stages = overviewLifecycleRibbon({
    intentOk: readiness?.intentOk ?? false,
    settlementOk: readiness?.settlementOk ?? false,
    dispatched,
  })
  ...
  return (
    <div className="mx-auto max-w-[1280px] space-y-4 bg-[#F8FAFC]">
