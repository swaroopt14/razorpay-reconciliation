...
export function BatchIntakePanel({
  ...
  return (
    <div className="space-y-4">
      ...
      {(intakeStep === 'intent_uploading' || intakeStep === 'settlement_uploading') && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="mx-4 w-full max-w-sm rounded-2xl border border-[#e5e5e5] bg-white px-8 py-6 shadow-xl">
            <p className="text-[16px] font-semibold text-[#0B1324]">
              {intakeStep === 'intent_uploading' ? 'Processing intents…' : 'Processing settlement…'}
            </p>
            <div className="mt-3 h-2.5 w-full overflow-hidden rounded-full bg-[#E2E8F0]">
              <div className="h-full w-full animate-[indeterminate_1.8s_ease-in-out_infinite] rounded-full bg-gradient-to-r from-[#3B82F6] via-[#6366F1] to-[#3B82F6]" />
            </div>
            <p className="mt-2 text-[12px] text-[#64748B]">
              {intakeStep === 'intent_uploading'
                ? 'Uploading and parsing payment instructions…'
                : 'Uploading and verifying settlement confirmation…'}
            </p>
          </div>
        </div>
      )}
