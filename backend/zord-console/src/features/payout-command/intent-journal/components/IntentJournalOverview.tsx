...
import { CROSS_BORDER_TRACE_ID, SCENARIO_CROSS_BORDER, getStoredScenario, withScenarioScope, type ConsoleScenario } from '@/services/payout-command/demo/scenarioMode'
...
type IntentJournalOverviewProps = {
  ...
  actions?: {
    ...
    onOpenReview?: () => void
    onCreatePolicy?: () => void
    /** Direct Policy Studio href — preferred over onClick for reliable navigation. */
...
export function IntentJournalOverview({
  ...
  const { ready, readiness, require } = useDemoBatchReady(undefined, {
  ...
  function dispatchBatch(batchId: string) {
    ...
    if (scenario === SCENARIO_CROSS_BORDER) {
      window.location.href = withScenarioScope(
        withDemoBatchScope(`/actions/${CROSS_BORDER_TRACE_ID}/dispatch`, batchId),
        SCENARIO_CROSS_BORDER,
      )
    } else {
      window.location.href = withDemoBatchScope('/execution/dispatches', batchId)
    }
  }
        ...
        <div className="flex flex-wrap gap-2">
          {scenario === SCENARIO_CROSS_BORDER ? (
            <Link
              ...
              onClick={(e) => {
                // Leaving /sandbox soft-nav often stalls — force the Policy Studio route.
                ...
                {batches.map((batch) => {
                  ...
                  return (
                    <li key={batch.batchId} className="flex items-center gap-4 bg-white px-5 py-3.5 transition hover:bg-[#FAFAFA]">
                      ...
                      {dispatchedBatchId === batch.batchId ? (
                        <Link
                          href={scenario === SCENARIO_CROSS_BORDER
                            ? withScenarioScope(
                                withDemoBatchScope(`/actions/${CROSS_BORDER_TRACE_ID}/dispatch`, batch.batchId),
                                SCENARIO_CROSS_BORDER,
                              )
                            : withDemoBatchScope('/execution/dispatches', batch.batchId)}
