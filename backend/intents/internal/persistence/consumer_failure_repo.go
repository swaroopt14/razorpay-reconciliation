...
// ConsumerFailureRepository persists R-03's durable Kafka failure ledger.
...
func (r *ConsumerFailurePostgresRepo) Record(ctx context.Context, rec models.ConsumerFailureReceipt) error {
	...
	`,
		...
		nullableString(rec.TenantID), nullableString(rec.TraceID), nullableString(rec.RequestID),
...
// using this codebase's existing sha256.Sum256 + hex.EncodeToString
// convention (see internal/services/intent_service.go).
...
// back to "topic:partition:offset" — per R-03's required idempotency key.
