...
// for the full background. This is zord-relay's copy of the same two
// tests: no raw version literals in production code, and this repo's
...
// empty-string literal (e.g. worker/processor.go's RouteKey{EventVersion:
// "", ...} for OutboxEvent/EdgeOutboxEvent) is a deliberate "this struct
// has no such field upstream" placeholder, not a hardcoded version value --
// a separate, already-scoped-out architectural gap (see the INT-07 plan),
// not the literal-vs-constant drift this test guards against.
...
// exercise mismatch/edge-case branches (e.g. worker/route_guard_test.go's
// SchemaVersion: "v2" mismatch case).
func TestINT07_NoRawVersionLiterals(t *testing.T) {
	root := int07FindModuleRoot(t)
	violations := int07FindRawVersionLiterals(t, root)
	if len(violations) > 0 {
		t.Fatalf("found %d raw SchemaVersion/EventVersion string literal(s) -- use the local EventVersionV1/SchemaVersionV1 constant (model/event_contract.go) instead:\n%s",
...
func TestINT07_LocalConstantsMatchSpec(t *testing.T) {
	...
	if model.EventVersionV1 != spec.EventVersionV1 {
		t.Fatalf("model.EventVersionV1 = %q, shared spec says %q", model.EventVersionV1, spec.EventVersionV1)
	}
	if model.SchemaVersionV1 != spec.SchemaVersionV1 {
		t.Fatalf("model.SchemaVersionV1 = %q, shared spec says %q", model.SchemaVersionV1, spec.SchemaVersionV1)
	}
	t.Logf("CONFIRMED: EventVersionV1=%q SchemaVersionV1=%q match the shared spec.", model.EventVersionV1, model.SchemaVersionV1)
}
