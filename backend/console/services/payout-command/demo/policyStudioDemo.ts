...
export type PolicyDraftBrief = {
  ...
  businessNote?: string
  /** Settlement currency scope used while drafting (drives approved rails). */
  settlementCurrency?: string
  /** Approved rails for the draft (e.g. NEFT,RTGS or SWIFT,UPI_XB). */
  ...
  impactNote: string
  /** Present when the undersettle-A/B/C premade prompt drafted this version. */
  undersettleSchedule?: {
    policyId: string
    taxBps: number
    marginBps: number
    companies: { code: string; legalName: string; orderRef: string }[]
  }
}
...
const ENTERPRISE_ACTIVE_RULES: PolicyRule[] = [
  ...
  rule({
    ...
    effect: 'block',
    pattern: 'If payment rail is not in [NEFT, RTGS, IMPS] → stop release',
    businessLabel: 'Approved domestic rails only (NEFT / RTGS / IMPS)',
  }),
  rule({
    id: 'r-ent-rpl-01',
    category: 'replay',
    ...
    versions: [
      {
        ...
        rules: [
          ...
          rule({
            ...
            value: 'SWIFT,UPI_XB',
            effect: 'block',
            pattern: 'If payment rail is not in [SWIFT, UPI (Cross-Border)] → stop release',
            businessLabel: 'Approved international rails only (SWIFT / UPI Cross-Border)',
          }),
          rule({
            id: 'r-xb-com-01',
            category: 'commercial',
    ...
    citations: [
      {
        ...
        detail: '97 of 100 payouts passed source, beneficiary, and commercial checks without a queue item.',
      },
      {
        label: 'Needs review',
...
export const POLICY_STUDIO_LINKS = {
  ...
  intentJournal: `/sandbox?dock=grid&demo=sandbox&batch_id=${DEMO_SMOKE_BATCH_ID}&client_batch_id=${DEMO_SMOKE_BATCH_ID}`,
  dispatch: `/actions/${CROSS_BORDER_TRACE_ID}/dispatch?demo=sandbox&batch_id=${DEMO_SMOKE_BATCH_ID}&scenario=cross-border`,
} as const

/** Deep-link into Intent Journal with a specific instruction selected. */
export function policyStudioIntentHref(intentId: string): string {
  ...
  if (id) q.set('intent_id', id)
  return `/payouts/intents?${q.toString()}`
}
