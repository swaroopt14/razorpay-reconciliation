...
import {
  ...
  UNDERSETTLE_POLICY_RULES,
  UNDERSETTLE_PROMPT,
  UNDERSETTLE_PROMPT_ID,
  UNDERSETTLE_PROMPT_LABEL,
  bpsLabel,
  undersettleDraftSchedule,
...
type CreatePolicyGuideDrawerProps = {
  ...
  /** Premade prompt id — applying it drafts the undersettle schedule without the wizard. */
  initialPromptId?: string | null
  /** Cross-border sandbox only. */
  enableUndersettlePrompt?: boolean
  onClose: () => void
...
const CONTROLS: {
  ...
  },
  {
    ...
    blurb: 'Block releases on rails your treasury has not approved for this currency.',
    effect: 'block',
    category: 'route',
    whenField: 'rail',
    operator: 'is not in',
    value: 'approved_rails',
    pattern: () => 'If payment rail is not in the approved list → stop release',
  },
]

/** Currency → suggested rails for policy drafting. */
export const SETTLEMENT_CURRENCIES = [
  { id: 'INR', label: 'INR · India (domestic)', scope: 'domestic' as const },
  { id: 'USD', label: 'USD · United States', scope: 'international' as const },
  { id: 'EUR', label: 'EUR · Eurozone', scope: 'international' as const },
  { id: 'AED', label: 'AED · UAE', scope: 'international' as const },
  { id: 'SGD', label: 'SGD · Singapore', scope: 'international' as const },
  { id: 'GBP', label: 'GBP · United Kingdom', scope: 'international' as const },
] as const

export type SettlementCurrencyId = (typeof SETTLEMENT_CURRENCIES)[number]['id']

export const DOMESTIC_RAILS = [
  {
    id: 'NEFT',
    label: 'NEFT',
    blurb: 'Domestic batch system — cannot directly touch foreign accounts.',
  },
  {
    id: 'RTGS',
    label: 'RTGS',
    blurb: 'Domestic real-time high-value — India only.',
  },
  {
    id: 'IMPS',
    label: 'IMPS',
    blurb: 'Domestic real-time — India only; can feed licensed cross-border operators.',
  },
] as const

export const INTERNATIONAL_RAILS = [
  {
    id: 'SWIFT',
    label: 'SWIFT',
    blurb: 'Primary rail for cross-border transfers via correspondent banking.',
  },
  {
    id: 'UPI_XB',
    label: 'UPI (Cross-Border)',
    blurb: 'Limited cross-border via UPI One World / bilateral linkages (e.g. Nepal, Singapore, UAE).',
  },
] as const

export function isDomesticSettlementCurrency(currency: string): boolean {
  return currency.toUpperCase() === 'INR'
}

export function suggestedRailsForCurrency(currency: string) {
  return isDomesticSettlementCurrency(currency) ? [...DOMESTIC_RAILS] : [...INTERNATIONAL_RAILS]
}

export function defaultCurrencyForPayoutKind(kind: PayoutKind): SettlementCurrencyId {
  return kind === 'cross_border' ? 'USD' : 'INR'
}
...
export function CreatePolicyGuideDrawer({
  ...
  const [settlementCurrency, setSettlementCurrency] = useState<SettlementCurrencyId>('INR')
  const [selectedRails, setSelectedRails] = useState<string[]>(() =>
    DOMESTIC_RAILS.map((r) => r.id),
  )
  ...
  const [includeUndersettle, setIncludeUndersettle] = useState(false)

  function applyUndersettlePrompt() {
    const ccy = defaultCurrencyForPayoutKind('cross_border')
    const rails = suggestedRailsForCurrency(ccy).map((r) => r.id)
    setKind('cross_border')
    setSettlementCurrency(ccy)
    setSelectedRails(rails)
    setSelectedControls((prev) =>
      prev.includes('unknown_rail') ? prev : [...prev, 'unknown_rail'],
    )
    setNote(UNDERSETTLE_PROMPT)
    setIncludeKindExtras(true)
    setIncludeUndersettle(true)
  }
  ...
  useEffect(() => {
    ...
    setSelectedRails(DOMESTIC_RAILS.map((r) => r.id))
    setSelectedControls(['beneficiary_change', 'high_amount', 'unknown_rail'])
    setThresholdLakh('5')
    setApproverRole('treasury')
    setActivatorRole('compliance')
    setNote('')
    setIncludeKindExtras(true)
    setIncludeUndersettle(false)
    if (initialPromptId === UNDERSETTLE_PROMPT_ID) {
      const ccy = defaultCurrencyForPayoutKind('cross_border')
      setKind('cross_border')
      setSettlementCurrency(ccy)
      setSelectedRails(suggestedRailsForCurrency(ccy).map((r) => r.id))
      setSelectedControls(['beneficiary_change', 'high_amount', 'unknown_rail'])
      setNote(UNDERSETTLE_PROMPT)
      setIncludeKindExtras(true)
      setIncludeUndersettle(true)
  ...
  const suggestedRails = useMemo(
    () => suggestedRailsForCurrency(settlementCurrency),
    [settlementCurrency],
  )
  const domesticScope = isDomesticSettlementCurrency(settlementCurrency)
  const approvedRailsLabel = selectedRails
    .map((id) => suggestedRails.find((r) => r.id === id)?.label ?? id)
    .join(', ')

  function selectPayoutKind(next: PayoutKind) {
    setKind(next)
    const ccy = defaultCurrencyForPayoutKind(next)
    setSettlementCurrency(ccy)
    const rails = suggestedRailsForCurrency(ccy).map((r) => r.id)
    setSelectedRails(rails)
    setSelectedControls((prev) =>
  ...
  function selectCurrency(ccy: SettlementCurrencyId) {
    setSettlementCurrency(ccy)
    setSelectedRails(suggestedRailsForCurrency(ccy).map((r) => r.id))
    setSelectedControls((prev) =>
      prev.includes('unknown_rail') ? prev : [...prev, 'unknown_rail'],
  ...
  function toggleRail(railId: string) {
    setSelectedRails((prev) => {
      const next = prev.includes(railId) ? prev.filter((x) => x !== railId) : [...prev, railId]
  ...
  const builtRules = useMemo(() => {
      ...
      (c, i) => {
        if (c.id === 'unknown_rail') {
          return {
            ...
            value: railsValue,
            effect: c.effect,
            pattern: `If payment rail is not in [${approvedRailsLabel || railsValue}] → stop release`,
            businessLabel: domesticScope
              ? `Approved domestic rails only (${approvedRailsLabel || 'NEFT / RTGS / IMPS'})`
              : `Approved international rails only (${approvedRailsLabel || 'SWIFT / UPI Cross-Border'})`,
          }
        }
        return {
          id: `r-zord-${c.id}-${i}`,
    ...
    const extras = includeKindExtras
      ...
      : []
    const undersettle = includeUndersettle ? UNDERSETTLE_POLICY_RULES : []
    return [...fromControls, ...extras, ...undersettle]
  }, [
    selectedControls,
    thresholdLakh,
    kind,
  ...
  function finish() {
    ...
    const purpose = [
      ...
      selectedRails.length
        ? `Approved rails: ${approvedRailsLabel}.`
        : '',
    ...
    const draftBrief: PolicyDraftBrief = {
      ...
      impactNote: includeUndersettle
        ? 'Sandbox mock: Apex (PAY-0001) and Summit (PAY-0004) match sealed net (Exact). Northwind (PAY-0002) remains ₹125 short of sealed net after authorised tax and margin. Those authorised lines are not settlement exceptions.'
        : `On the sample batch this draft would roughly: allow ${impactPreview.wouldAllow}, warn ${impactPreview.wouldWarn}, need approval ${impactPreview.wouldNeedApproval}, block ${impactPreview.wouldBlock}.`,
      undersettleSchedule: includeUndersettle ? undersettleDraftSchedule() : undefined,
    }

    const agentSummary = includeUndersettle
      ? `Zord drafted a cross-border incomplete-order net policy for Apex, Northwind, and Summit (${builtRules.length} controls). Authorised tax and margin seal on the contract; they are not settlement leakage.`
      : `Zord saved a draft for ${kindMeta.label} (${builtRules.length} controls, ${settlementCurrency} · ${approvedRailsLabel || 'rails TBD'}). ${approverLabel} reviews exceptions; only ${activatorLabel} can make it live.`
    ...
    onComplete({
      ...
      mandatoryArtifacts: includeUndersettle
        ? [...kindMeta.artifacts, 'order fulfilment status']
        : kindMeta.artifacts,
      agentSummary,
      draftBrief,
      impactPreview,
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="border border-[#E2E8F0] px-3 py-2.5">
                  ...
                  <p className="mt-1 text-[13px] font-semibold text-[#0B1324]">{settlementCurrency}</p>
                  <p className="mt-0.5 text-[11px] text-[#64748B]">
                    {domesticScope ? 'Domestic India rails' : 'International rails only'}
                  </p>
                </div>
                <div className="border border-[#E2E8F0] px-3 py-2.5">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[#94A3B8]">
              ...
              {includeUndersettle ? (
                <div className="border border-[#E2E8F0] bg-[#F8FAFC] px-3 py-2.5">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[#94A3B8]">
                    Incomplete-order net schedule · sandbox
                  </p>
                  <p className="mt-1 text-[12px] leading-relaxed text-[#475569]">
                    Invoice minus {bpsLabel(undersettleDraftSchedule().taxBps)} tax and{' '}
                    {bpsLabel(undersettleDraftSchedule().marginBps)} margin becomes the sealed
                    expected net. Bank credits that match this net are Exact.
                  </p>
                  <ul className="mt-2 space-y-1.5 text-[12px] text-[#0B1324]">
                    {UNDERSETTLE_COMPANIES.map((c) => (
                      <li key={c.code} className="flex justify-between gap-3">
                        <span>
                          Company {c.code} · {c.legalName}
                        </span>
                        <span className="shrink-0 text-[#64748B]">{c.orderRef}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <button
                  type="button"
                  onClick={() => {
                    applyUndersettlePrompt()
                    runAgentDraft()
                  }}
                  className="w-full border border-[#0B1324] bg-[#F8FAFC] px-3.5 py-3 text-left transition hover:bg-[#F1F5F9]"
                >
                  ...
                  <div className="space-y-2">
                    {PAYOUT_KINDS.map((k) => (
                      ...
                      >
                        ...
                        {k.id === 'cross_border' ? (
                          <span className="mt-1 block text-[11px] font-medium text-[#475569]">
                            Suggests international rails (SWIFT / UPI Cross-Border) — not NEFT/RTGS/IMPS
                          </span>
                        ) : null}
                      </button>
                    ))}
              ...
              {step === 2 ? (
                <div className="space-y-4">
                  <div className="space-y-3">
                    <p className="text-[14px] font-semibold text-[#0B1324]">
                      Settlement currency & approved rails
                    </p>
                    <p className="text-[12px] leading-relaxed text-[#64748B]">
                      {domesticScope
                        ? 'INR uses domestic India rails (NEFT / RTGS / IMPS). They cannot directly touch foreign accounts.'
                        : 'Non-INR settlement suggests international rails only — SWIFT and UPI (Cross-Border). Indian domestic rails are not offered.'}
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {SETTLEMENT_CURRENCIES.map((c) => (
                        <button
                          ...
                          className={`border px-3 py-2 text-left text-[12px] font-semibold transition ${
                            settlementCurrency === c.id
                              ? 'border-[#0B1324] bg-[#0B1324] text-white'
                              : 'border-[#E2E8F0] bg-white text-[#0B1324] hover:border-[#CBD5E1]'
                          }`}
                        >
                          {c.id}
                          <span
                            className={`mt-0.5 block text-[10px] font-medium ${
                              settlementCurrency === c.id ? 'text-white/70' : 'text-[#94A3B8]'
                            }`}
                          >
                            {c.scope === 'domestic' ? 'Domestic' : 'International'}
                          </span>
                        </button>
                      ))}
                    </div>
                    <div className="space-y-2">
                      {suggestedRails.map((rail) => {
                        const on = selectedRails.includes(rail.id)
                        return (
                          <button
                            key={rail.id}
                            type="button"
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      {CONTROLS.map((c) => {
                        ...
                        const railHint =
                          c.id === 'unknown_rail' && approvedRailsLabel
                            ? `Approved for ${settlementCurrency}: ${approvedRailsLabel}`
                            : null
                        return (
                          <button
                            key={c.id}
              ...
              {step === 3 ? (
                <div className="space-y-4">
                  ...
                  <div className="border border-[#E2E8F0] bg-[#F8FAFC] px-3 py-2.5 text-[12px] text-[#475569]">
                    ...
                    for {kindMeta.label.toLowerCase()} · {settlementCurrency} ·{' '}
                    {approvedRailsLabel || 'rails TBD'}.
                  </div>
                </div>
              ) : null}
            </div>
