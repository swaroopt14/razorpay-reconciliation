...
type TabId = 'timeline' | 'lifecycle' | 'signals' | 'attempts' | 'files' | 'tech'
...
export function PaymentTraceSurface({ paymentId }: { paymentId?: string }) {
  ...
  const [tab, setTab] = useState<TabId>(() => {
    ...
    return t === 'lifecycle' ? 'lifecycle' : 'timeline'
  })
  ...
  const sourceTraces = ready ? DEMO_PAYMENT_TRACES : []
  const batches = useMemo(() => buildTraceBatches(sourceTraces), [sourceTraces])
  const batchPayouts = useMemo(
    () => (openBatchId ? tracesForBatch(sourceTraces, openBatchId) : []),
    [openBatchId, sourceTraces],
  )
  const activeBatch = openBatchId
    ? (batches.find((b) => b.batchId === openBatchId) ?? null)
    : null
  const overviewStats = useMemo(() => traceOverviewStats(sourceTraces), [sourceTraces])
  ...
  function openPayment(id: string) {
    ...
    setActiveId(id)
    setView('detail')
    setTab(searchParams.get('tab') === 'lifecycle' ? 'lifecycle' : 'timeline')
    setNotice(null)
...
function DetailView({
  ...
  return (
    <div className={`mx-auto space-y-5 ${tab === 'lifecycle' ? 'max-w-[1200px]' : 'max-w-[880px]'}`}>
      ...
      {tab === 'lifecycle' ? (
        <div className="overflow-hidden border border-[#E5E5E5] bg-white">
          <LifecycleGraphSurface traceId={CROSS_BORDER_TRACE_ID} embedded />
        </div>
      ) : null}
