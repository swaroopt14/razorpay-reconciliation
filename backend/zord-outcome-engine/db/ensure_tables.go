...
func EnsureTables(ctx context.Context) error {
	...
	stmts := []string{
		...
		`CREATE INDEX IF NOT EXISTS canonical_intents_tenant_idx ON canonical_intents(tenant_id);`,
		`CREATE INDEX IF NOT EXISTS canonical_intents_client_ref_idx ON canonical_intents(client_payout_ref) WHERE client_payout_ref IS NOT NULL;`,
		`CREATE INDEX IF NOT EXISTS canonical_intents_amount_currency_idx ON canonical_intents(tenant_id, amount, currency_code);`,
		`CREATE INDEX IF NOT EXISTS canonical_intents_client_batch_ref_idx ON canonical_intents(tenant_id, client_batch_ref) WHERE client_batch_ref IS NOT NULL;`,
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS settlement_batches_tenant_psp_client_uq ON settlement_batches(tenant_id, psp, client_batch_id);`,
		`CREATE INDEX IF NOT EXISTS settlement_batches_tenant_idx ON settlement_batches(tenant_id);`,
		...
		`CREATE INDEX IF NOT EXISTS settlement_ingest_runs_batch_idx ON settlement_ingest_runs(settlement_batch_id);`,
		`CREATE INDEX IF NOT EXISTS settlement_ingest_runs_tenant_idx ON settlement_ingest_runs(tenant_id);`,
		`CREATE INDEX IF NOT EXISTS settlement_ingest_runs_status_idx ON settlement_ingest_runs(run_status);`,
		`CREATE INDEX IF NOT EXISTS settlement_ingest_runs_envelope_idx ON settlement_ingest_runs(settlement_envelope_id);`,
		...
		`CREATE INDEX IF NOT EXISTS canonical_obs_tenant_run_idx ON canonical_settlement_observations(tenant_id, ingest_run_id);`,
		`CREATE INDEX IF NOT EXISTS canonical_obs_batch_idx ON canonical_settlement_observations(settlement_batch_id);`,
		`CREATE INDEX IF NOT EXISTS canonical_obs_tenant_client_ref_lower_idx ON canonical_settlement_observations(tenant_id, LOWER(client_reference_candidate)) WHERE client_reference_candidate IS NOT NULL;`,
		`CREATE INDEX IF NOT EXISTS canonical_obs_tenant_batch_ref_lower_idx ON canonical_settlement_observations(tenant_id, LOWER(batch_reference)) WHERE batch_reference IS NOT NULL;`,
		`CREATE INDEX IF NOT EXISTS canonical_obs_tenant_client_batch_id_lower_idx ON canonical_settlement_observations(tenant_id, LOWER(client_batch_id)) WHERE client_batch_id IS NOT NULL AND client_batch_id <> '';`,
		`CREATE INDEX IF NOT EXISTS canonical_obs_tenant_curr_amt_ts_idx ON canonical_settlement_observations(tenant_id, currency_code, amount, observation_timestamp);`,
		...
		// FIX #14: stale_after column added so a background goroutine can detect
		// jobs stuck RUNNING when the EC2 instance was replaced mid-flight.
		...
		`CREATE INDEX IF NOT EXISTS attachment_jobs_tenant_idx ON attachment_jobs(tenant_id);`,
		`CREATE INDEX IF NOT EXISTS attachment_jobs_status_idx ON attachment_jobs(status);`,
		...
		`CREATE INDEX IF NOT EXISTS attachment_candidates_observation_idx ON attachment_candidates(settlement_observation_id);`,
		`CREATE INDEX IF NOT EXISTS attachment_candidates_intent_idx ON attachment_candidates(intent_id);`,
		`CREATE INDEX IF NOT EXISTS attachment_candidates_job_idx ON attachment_candidates(attachment_job_id);`,
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS attachment_decisions_intent_uq ON attachment_decisions(intent_id, attachment_job_id);`,
		`CREATE UNIQUE INDEX IF NOT EXISTS attachment_decisions_observation_job_uq ON attachment_decisions(settlement_observation_id, attachment_job_id) WHERE settlement_observation_id IS NOT NULL;`,
		`CREATE INDEX IF NOT EXISTS attachment_decisions_observation_idx ON attachment_decisions(settlement_observation_id) WHERE settlement_observation_id IS NOT NULL;`,
		`CREATE INDEX IF NOT EXISTS attachment_decisions_intent_idx ON attachment_decisions(intent_id) WHERE intent_id IS NOT NULL;`,
		`CREATE INDEX IF NOT EXISTS attachment_decisions_type_idx ON attachment_decisions(decision_type);`,
		`CREATE INDEX IF NOT EXISTS attachment_decisions_obs_tenant_strong_match_idx ON attachment_decisions(tenant_id, settlement_observation_id) WHERE settlement_observation_id IS NOT NULL AND decision_type IN ('MATCH_EXACT', 'MATCH_HIGH_CONFIDENCE');`,
		...
		`CREATE INDEX IF NOT EXISTS variance_records_decision_idx ON variance_records(attachment_decision_id);`,
		`CREATE INDEX IF NOT EXISTS variance_records_intent_idx ON variance_records(intent_id);`,
		`CREATE INDEX IF NOT EXISTS variance_records_severity_idx ON variance_records(variance_severity);`,
		`CREATE INDEX IF NOT EXISTS variance_records_whitelisted_idx ON variance_records(is_whitelisted);`,
		...
		`CREATE INDEX IF NOT EXISTS batch_attachment_summaries_tenant_idx ON batch_attachment_summaries(tenant_id);`,
		`CREATE INDEX IF NOT EXISTS batch_attachment_summaries_job_idx ON batch_attachment_summaries(attachment_job_id);`,
		...
		`CREATE INDEX IF NOT EXISTS attachment_outbox_status_idx ON attachment_outbox_events(status, next_retry_at);`,
		`CREATE INDEX IF NOT EXISTS attachment_outbox_job_idx ON attachment_outbox_events(attachment_job_id);`,
		...
		`CREATE INDEX IF NOT EXISTS outcome_outbox_status_idx ON outcome_outbox(status, next_attempt_at);`,
		`CREATE INDEX IF NOT EXISTS outcome_outbox_tenant_idx ON outcome_outbox(tenant_id);`,
		...
		`CREATE INDEX IF NOT EXISTS unresolved_intent_records_tenant_idx ON unresolved_intent_records(tenant_id);`,
		`CREATE INDEX IF NOT EXISTS unresolved_intent_records_job_idx ON unresolved_intent_records(attachment_job_id);`,
		`CREATE INDEX IF NOT EXISTS unresolved_intent_records_intent_idx ON unresolved_intent_records(intent_id);`,
		`CREATE INDEX IF NOT EXISTS unresolved_intent_records_batch_idx ON unresolved_intent_records(batch_id) WHERE batch_id IS NOT NULL;`,
		...
		`CREATE INDEX IF NOT EXISTS ambiguous_intent_records_tenant_idx ON ambiguous_intent_records(tenant_id);`,
		`CREATE INDEX IF NOT EXISTS ambiguous_intent_records_job_idx ON ambiguous_intent_records(attachment_job_id);`,
		`CREATE INDEX IF NOT EXISTS ambiguous_intent_records_intent_idx ON ambiguous_intent_records(intent_id);`,
		`CREATE INDEX IF NOT EXISTS ambiguous_intent_records_batch_idx ON ambiguous_intent_records(batch_id) WHERE batch_id IS NOT NULL;`,
		...
		`CREATE INDEX IF NOT EXISTS conflicted_intent_records_tenant_idx ON conflicted_intent_records(tenant_id);`,
		`CREATE INDEX IF NOT EXISTS conflicted_intent_records_job_idx ON conflicted_intent_records(attachment_job_id);`,
		`CREATE INDEX IF NOT EXISTS conflicted_intent_records_intent_idx ON conflicted_intent_records(intent_id);`,
		`CREATE INDEX IF NOT EXISTS conflicted_intent_records_batch_idx ON conflicted_intent_records(batch_id) WHERE batch_id IS NOT NULL;`,
		...
		`CREATE INDEX IF NOT EXISTS orphan_settlement_records_tenant_idx ON orphan_settlement_records(tenant_id);`,
		`CREATE INDEX IF NOT EXISTS orphan_settlement_records_job_idx ON orphan_settlement_records(attachment_job_id);`,
		`CREATE INDEX IF NOT EXISTS orphan_settlement_records_obs_idx ON orphan_settlement_records(settlement_observation_id);`,
		`CREATE INDEX IF NOT EXISTS orphan_settlement_records_batch_idx ON orphan_settlement_records(batch_id) WHERE batch_id IS NOT NULL;`,
		...
		// FIX #14: add stale_after to existing attachment_jobs rows
...
// SeedDefaultAttachmentRuleProfile upserts the default matching ruleset for a tenant.
// FIX #18: call this at startup or tenant provisioning only — NOT on every request.
// The handler layer uses a sync.Map cache to ensure this is called at most once
...
func SeedDefaultAttachmentRuleProfile(ctx context.Context, tenantID interface{}) error {
  ...
  "version": "v1",
  "exact_ref_priority_json": {"priority_order":["zord_signature","client_payout_ref","provider_reference","bank_reference"]},
  "carrier_priority_json": {"exact_ref":120,"client_ref":100,"provider_ref":85,"bank_ref":85,"zord_signature":120,"beneficiary_match":35,"amount_match":30,"currency_match":10,"batch_match":90,"time_window":20,"source_system":10,"parse_confidence_modifier":-20,"source_strength_modifier":-15,"conflict_penalty":-70},
  "time_window_policy_json": {"max_hours_difference":72,"strict_same_day":false,"allow_cross_period":true},
  "amount_tolerance_policy_json": {"exact_match_required":true,"tolerance_minor":0,"allow_percentage_tolerance":false,"percentage_tolerance":0},
  "batch_boundary_policy_json": {"strict_batch_matching":false,"allow_cross_batch_if_strong_match":true},
  "manual_review_thresholds_json": {"high_confidence_score":135,"exact_match_score":200,"ambiguity_margin_threshold":15,"min_score_for_auto_attach":80,"max_candidates_for_auto_attach":1},
  ...
  "status": "ACTIVE"
	...
	_, err := DB.ExecContext(ctx, `
		INSERT INTO attachment_rule_profiles (
			...
			ambiguity_margin_threshold, requires_bank_ref_for_exact_flag, status
		...
		ruleset.AmbiguityMarginThreshold, ruleset.RequiresBankRefForExactFlag, ruleset.Status,
	)
	return err
}
