...
func TestTOK06_RepoActiveKeyIsolatedPerTenant(t *testing.T) {
	...
	if rotated, err := repo.RotateKey(ctx, tenantA, keyAv1, make([]byte, 32), "test"); err != nil || !rotated {
		t.Fatalf("RotateKey(tenant A, initial) rotated=%v err = %v", rotated, err)
	}
	if rotated, err := repo.RotateKey(ctx, tenantB, keyBv1, make([]byte, 32), "test"); err != nil || !rotated {
	...
	if rotated, err := repo.RotateKey(ctx, tenantA, keyAv2, make([]byte, 32), "test"); err != nil || !rotated {
