...
func SaveRawIntent(
	...
	outboxQuery := `
		INSERT INTO ingress_outbox
		(trace_id, envelope_id, tenant_id, artifact_id, artifact_version_id, object_ref, received_at, ingress_channel, source, idempotency_key, encrypted_payload, payload_hash, envelope_hash, envelope_signature, topic, status, lease_id, event_type, lease_until, created_at, updated_at, published_at, failure_reason_code, batchid, file_content_hash, source_row_ref, source_system)
	...
	_, err = tx.ExecContext(ctx, outboxQuery,
		...
		envelope.EnvelopeHash,      // $13 (hex)
		envelope.EnvelopeSignature, // $14 (string -> TEXT)
		topic,                      // $15
		"PENDING",                  // $16 (status)
		envelope.LeaseID,           // $17
		envelope.EventType,         // $18
		envelope.LeaseUntil,        // $19
		envelope.CreatedAt,         // $20
		envelope.UpdatedAt,         // $21
		envelope.PublishedAt,       // $22
		envelope.FailureReasonCode, // $23
		envelope.BatchID,           // $24
		envelope.FileContentHash,   // $25
		envelope.SourceRowRef,      // $26
		envelope.SourceSystem,      // $27
	)
	if err != nil {
